...
int main()
{
	...
	printf("TEST convert utf8to16\n");
	{
		for (int i = 0;i < sizeof(unicode_test_data) / sizeof(unicode_test_data[0]);i++)
		{
			...
			n8 = tuple.utf8.size();
			nbytes = utf8util::utf8to16_bytes((uint8_t*)tuple.utf8.c_str(), n8);
			n16 = nbytes / 2;memset(d16, 0xF, sizeof(d16));
			success = utf8util::utf8to16((uint8_t*)tuple.utf8.c_str(), n8, d16, n16) == RESULT_OK;
			res.assign((const wchar_t*)d16, n16);
			success = success && res == tuple.utf16 && (nbytes & 1) == 0;
			printf("%i) %s\n", i, success ? "success" : "error");
		}
	}
	printf("TEST convert utf8to32\n");
	{
		for (int i = 0;i < sizeof(unicode_test_data) / sizeof(unicode_test_data[0]);i++)
		{
			...
			tuple = (unicode_tuple&)unicode_test_data[i];
			n8 = tuple.utf8.size();
			nbytes = utf8util::utf8to32_bytes((uint8_t*)tuple.utf8.c_str(), n8);
			n32 = nbytes / 4;memset(d32, 0xF, sizeof(d32));
			success = utf8util::utf8to32((uint8_t*)tuple.utf8.c_str(), n8, d32, n32) == RESULT_OK;
			res.assign((const char32_t*)d32, n32);
			success = success && res == tuple.utf32 && (nbytes & 3) == 0;
			printf("%i) %s\n", i, success ? "success" : "error");
		}
	}
	printf("TEST convert utf16to8\n");
	{
		for (int i = 0;i < sizeof(unicode_test_data) / sizeof(unicode_test_data[0]);i++)
		{
			...
			nbytes = utf8util::utf16to8_bytes((uint16_t*)tuple.utf16.c_str(), n16);
			n8 = nbytes;memset(d8, 0xF, sizeof(d8));
			success = utf8util::utf16to8((uint16_t*)tuple.utf16.c_str(), n16, d8, n8) == RESULT_OK;
			res.assign((const char*)d8, n8);
			success = success && res == tuple.utf8;
			printf("%i) %s\n", i, success ? "success" : "error");
		}
	}
	printf("TEST convert utf32to8\n");
	{
		for (int i = 0;i < sizeof(unicode_test_data) / sizeof(unicode_test_data[0]);i++)
		{
			...
			nbytes = utf8util::utf32to8_bytes((uint32_t*)tuple.utf32.c_str(), n32);
			n8 = nbytes;memset(d8, 0xF, sizeof(d8));
			success = utf8util::utf32to8((uint32_t*)tuple.utf32.c_str(), n32, d8, n8) == RESULT_OK;
			res.assign((const char*)d8, n8);
			success = success && res == tuple.utf8;
			printf("%i) %s\n", i, success ? "success" : "error");
		}
